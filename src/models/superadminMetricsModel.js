...
const getKPIsByPeriod = async (days) => {
  const queries = {
    ...
    `,
    pendingTasks: `
      SELECT COUNT(*) as total FROM Tareas WHERE estatus = 'Pendiente'
    `,
    overdueTasks: `
      SELECT COUNT(*) as total FROM Tareas
      WHERE estatus != 'Completada' AND fecha_vencimiento < NOW()
    `
  };
  ...
  const [newProjects, completedTasks, signedDocuments, pendingTasks, overdueTasks] = await Promise.all([
    ...
    executeQuery(queries.pendingTasks),
    executeQuery(queries.overdueTasks)
  ]);
...
// Estadísticas de documentos firmados vs no firmados (global)
const getDocumentsSignedStats = async (projectId = null) => {
  let projectFilter = '';
  const params = [];
